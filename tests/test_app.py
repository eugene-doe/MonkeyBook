...
class TestClass:
    ...
    def test_get_monkey_by_email(session):
        monkey = Monkey('Test', 'User', '123', 'test@email.com', str(date.today()))
        db.session.add(monkey)
        db.session.commit()
        ...
        assert type(same_monkey) is Monkey
        assert same_monkey.email == monkey.email

    def test_get_monkey_by_id(session):
        monkey = Monkey('Test', 'User', '123', 'test@email.com', str(date.today()))
        db.session.add(monkey)
        db.session.commit()
    ...
    def test_check_unique_email(session):
        monkey = Monkey('Test', 'User', '123', 'test@email.com', str(date.today()))
        db.session.add(monkey)
        db.session.commit()

        assert dba.check_unique_email(monkey.email) == False
        assert dba.check_unique_email(monkey.email, monkey.id) == True

    def test_register_monkey(session):
        ...
        assert type(monkey) is Monkey
        assert monkey.id > 0

    def test_delete_monkey(session):
        monkey = Monkey('Test', 'User', '123', 'test@email.com', str(date.today()))
        db.session.add(monkey)
        db.session.commit()

        assert dba.delete_monkey(monkey) == True

    def test_get_monkey_profile(session):
        monkey = Monkey('Test', 'User', '123', 'test@email.com', str(date.today()))
        db.session.add(monkey)
        db.session.commit()
    ...
    def test_edit_monkey_profile(session):
        monkey = Monkey('Test', 'User', '123', 'test@email.com', str(date.today()))
        db.session.add(monkey)
        db.session.commit()
        ...
        assert dba.edit_monkey_profile(monkey, form) == True
    ...
    def test_add_friend(session):
        ...
        assert dba.add_friend(rich, bill) == True
    ...
    def test_remove_friend(session):
        ...
        assert bill in rich.friends
        assert rich in bill.friend_of
        assert dba.remove_friend(rich, bill) == True
    ...
    def test_set_best_friend(session):
        bill = Monkey('Bill', 'Gates', 'secret', 'billg@microsoft.com')
        rich = Monkey('Richard', 'Stallman', 'freedom', 'rms@gnu.org')
        rich.friends.append(bill)
        db.session.add(bill, rich)
        ...
        assert dba.set_best_friend(rich, bill) == True
    ...
    def test_clear_best_friend(session):
        bill = Monkey('Bill', 'Gates', 'secret', 'billg@microsoft.com')
        rich = Monkey('Richard', 'Stallman', 'freedom', 'rms@gnu.org')
        rich.friends.append(bill)
        ...
        assert dba.clear_best_friend(rich) == True
